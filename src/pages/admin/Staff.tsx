...
export default function Staff() {
  const { employees, services, appointments, addEmployee, updateEmployee, deleteEmployee } = useStore();
  ...
  async function handleDelete(id: string) {
    ...
    if (activeCount > 0) {
      await swal.confirm({
        title: `Bu personelin ${activeCount} aktif randevusu var`,
        text: 'Silmeden önce randevuları başka personele aktarın.',
        confirmText: 'Anladım',
      });
      return;
    }
            ...
            <Card key={emp.id} className="group">
              ...
              <div className="flex items-start gap-3.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    <span className="text-xs text-gray-400">{emp.workingHours.start} – {emp.workingHours.end}</span>
                  </div>
                  {avgRating !== null && (
                    <div className="flex items-center gap-1 mt-0.5">
                      <Star size={10} className="fill-amber-400 text-amber-400" />
                      <span className="text-xs text-amber-500 font-semibold">{avgRating.toFixed(1)}</span>
              ...
              <div className="mt-3 flex flex-wrap gap-1">
                {empServices.slice(0, 3).map(s => (
                  <span key={s.id} className="rounded-md bg-[#EEF2FF] dark:bg-[#312E81]/30 px-1.5 py-0.5 text-[10px] font-medium text-[#6366F1] dark:text-indigo-400">
                    {s.name}
                  </span>
                ))}
                {empServices.length > 3 && (
                  <span className="rounded-md bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 text-[10px] font-medium text-gray-500">
                    +{empServices.length - 3}
                  </span>
                )}
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700/60 grid grid-cols-2 gap-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <div>
                    <p className="text-sm font-bold text-gray-900 dark:text-white tabular-nums">{completed}</p>
                    <p className="text-[10px] text-gray-400 uppercase tracking-wide">Tamamlanan</p>
                  </div>
                </div>
                <div className="flex items-center gap-1.5">
                  <TrendingUp size={13} className="text-[#6366F1] shrink-0" />
                  <div>
                    <p className="text-sm font-bold text-[#6366F1] dark:text-indigo-400 tabular-nums truncate">{revenue.toLocaleString('tr-TR')} ₺</p>
                    <p className="text-[10px] text-gray-400 uppercase tracking-wide">Gelir</p>
                  </div>
                </div>
              </div>
            </Card>
