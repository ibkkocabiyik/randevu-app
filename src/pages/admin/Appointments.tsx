    ...
    <form onSubmit={e => { e.preventDefault(); onSave(form); }} className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input required value={form.customerName} onChange={e => setF('customerName', e.target.value)} placeholder="Müşteri adı" className={inputCls} />
        </div>
        <div>
          <label className="mb-1.5 block text-[0.8125rem] font-medium text-gray-700 dark:text-white/60">Telefon</label>
          <input required type="tel" value={form.customerPhone} onChange={e => setF('customerPhone', e.target.value)} placeholder="0555 000 00 00" className={inputCls} />
        </div>
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label className="mb-1.5 block text-[0.8125rem] font-medium text-gray-700 dark:text-white/60">Hizmet</label>
          <select value={form.serviceId} onChange={e => setF('serviceId', e.target.value)} className={inputCls}>
            {services.map(s => <option key={s.id} value={s.id}>{s.name} – {s.price} ₺</option>)}
          </select>
        </div>
        <div>
          <label className="mb-1.5 block text-[0.8125rem] font-medium text-gray-700 dark:text-white/60">Personel</label>
          <select value={form.employeeId} onChange={e => setF('employeeId', e.target.value)} className={inputCls}>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div>
          ...
          <input type="date" required value={form.date} onChange={e => setF('date', e.target.value)} className={inputCls} />
        </div>
        <div>
          <label className="mb-1.5 block text-[0.8125rem] font-medium text-gray-700 dark:text-white/60">Başlangıç</label>
          <input type="time" required value={form.startTime} onChange={e => setF('startTime', e.target.value)} className={inputCls} />
        </div>
        <div>
          <label className="mb-1.5 block text-[0.8125rem] font-medium text-gray-700 dark:text-white/60">Bitiş</label>
          <input type="time" required value={form.endTime} onChange={e => setF('endTime', e.target.value)} className={inputCls} />
        </div>
      </div>
      <div>
        <label className="mb-1.5 block text-[0.8125rem] font-medium text-gray-700 dark:text-white/60">Durum</label>
        <select value={form.status} onChange={e => setF('status', e.target.value as Appointment['status'])} className={inputCls}>
      ...
      <div>
        ...
        <textarea rows={3} value={form.notes} onChange={e => setF('notes', e.target.value)} placeholder="Müşteri notu…" className={cn(inputCls, 'resize-none')} />
      </div>
...
function AppointmentDetail({ appt, onClose }: { appt: Appointment; onClose: () => void }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3"><StatusPill status={appt.status} /></div>
      ...
      <button onClick={onClose} className="w-full rounded-xl border border-gray-200 dark:border-gray-700 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Kapat</button>
    </div>
  );
}
...
export default function Appointments() {
  ...
  async function handleDelete(id: string) {
    ...
    catch { await useData.getState().fetchAppointments(); }
    swal.toast({ icon: 'success', title: 'Randevu silindi' });
  }
  ...
  async function handleSaveNew(data: Omit<Appointment, 'id'>) {
    try {
      ...
      swal.toast({ icon: 'error', title: (e as Error).message ?? 'Hata oluştu' });
      return;
    }
    setShowNew(false);
  ...
  async function handleSaveEdit(data: Omit<Appointment, 'id'>) {
    if (!editAppt) return;
    upsertAppointment({ ...editAppt, ...data });
    try {
      const api = await appointmentsApi.update(editAppt.id, {
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button onClick={() => setShowNew(true)} className="inline-flex items-center gap-2 rounded-xl bg-[#6366F1] px-4 py-2 text-sm font-medium text-white shadow-sm shadow-[#6366F1]/30 hover:bg-[#4F46E5] active:scale-[0.98] transition-all">
          <CirclePlus size={16} /> Yeni Randevu
        </button>
      </div>
      ...
      <Card padding="sm">
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Müşteri adı veya telefon…"
            ...
            {['all', 'confirmed', 'pending', 'completed', 'cancelled', 'noshow'].map(s => (
              <button key={s} onClick={() => setStatusFilter(s)}
                className={cn('rounded-lg px-3 py-1.5 text-xs font-medium transition-all',
                  statusFilter === s ? 'bg-[#6366F1] text-white shadow-sm' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600'
                )}>
                      ...
                      <tr key={appt.id} className="group hover:bg-gray-50/60 dark:hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => setDetailAppt(appt)} className="flex h-7 w-7 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-600 transition-colors"><Eye size={14} /></button>
                            <button onClick={() => setEditAppt(appt)} className="flex h-7 w-7 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-[#6366F1] transition-colors"><Pencil size={14} /></button>
                            <button onClick={() => handleDelete(appt.id)} className="flex h-7 w-7 items-center justify-center rounded-lg text-gray-400 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-500 transition-colors"><Trash2 size={14} /></button>
                          </div>
                        </td>
                      </tr>
                    );
