...
const CATEGORY_LABELS: Record<Service['category'], string> = { sac: 'Saç', sakal: 'Sakal', bakim: 'Bakım' };
    ...
    <form onSubmit={e => { e.preventDefault(); onSave(form); }} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input type="number" min={5} required value={form.durationMinutes} onChange={e => setForm(f => ({ ...f, durationMinutes: +e.target.value }))} className={inputCls} />
        </div>
        <div>
          <label className="mb-1.5 block text-[0.8125rem] font-medium text-gray-700 dark:text-white/60">Fiyat (₺)</label>
          <input type="number" min={0} required value={form.price} onChange={e => setForm(f => ({ ...f, price: +e.target.value }))} className={inputCls} />
        </div>
      </div>
      <div>
          ...
          {(['sac', 'sakal', 'bakim'] as Service['category'][]).map(cat => (
            <button key={cat} type="button" onClick={() => setForm(f => ({ ...f, category: cat }))}
              className={cn('flex-1 rounded-xl border py-2 text-sm font-medium transition-all',
                form.category === cat ? 'bg-[#6366F1] text-white border-[#6366F1] shadow-sm shadow-[#6366F1]/20' : 'bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-[#6366F1]/40'
              )}>
              {CATEGORY_LABELS[cat]}
            </button>
          ))}
...
function ReviewsModal({ reviews }: { reviews: Review[] }) {
        ...
        <div className="flex items-center gap-3 p-4 rounded-2xl bg-amber-50 dark:bg-amber-900/20">
          ...
          <div>
            <div className="flex gap-0.5">
              {[1,2,3,4,5].map(s => <Star key={s} size={16} className={cn(parseFloat(avg) >= s ? 'fill-amber-400 text-amber-400' : 'fill-gray-200 text-gray-200 dark:fill-gray-700 dark:text-gray-700')} />)}
            </div>
        ...
        {reviews.map(r => (
          <div key={r.id} className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/60 space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-0.5">{[1,2,3,4,5].map(s => <Star key={s} size={12} className={cn(r.rating >= s ? 'fill-amber-400 text-amber-400' : 'fill-gray-200 text-gray-200')} />)}</div>
            </div>
...
export default function Services() {
  ...
  async function handleDelete(id: string) {
    ...
    catch { await useData.getState().fetchServices(); }
    swal.toast({ icon: 'success', title: 'Hizmet silindi' });
  }

  async function handleSaveNew(data: Omit<Service, 'id'>) {
    try {
      const api = await servicesApi.create({ name: data.name, duration_minutes: data.durationMinutes, price: data.price, category: data.category });
      upsertService(toService(api));
    } catch (e: unknown) {
      swal.toast({ icon: 'error', title: (e as Error).message ?? 'Hata' }); return;
    }
    setShowNew(false);
    swal.toast({ icon: 'success', title: 'Hizmet eklendi' });
  }

  async function handleSaveEdit(data: Omit<Service, 'id'>) {
    if (!editSvc) return;
    upsertService({ ...editSvc, ...data });
    try {
      const api = await servicesApi.update(editSvc.id, { name: data.name, duration_minutes: data.durationMinutes, price: data.price, category: data.category });
      upsertService(toService(api));
    } catch { await useData.getState().fetchServices(); }
    setEditSvc(null);
    swal.toast({ icon: 'success', title: 'Hizmet güncellendi' });
  }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowNew(true)} className="inline-flex items-center gap-2 rounded-xl bg-[#6366F1] px-4 py-2 text-sm font-medium text-white shadow-sm shadow-[#6366F1]/30 hover:bg-[#4F46E5] active:scale-[0.98] transition-all">
          <CirclePlus size={16} /> Hizmet Ekle
        </button>
      </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-gray-700/40">
                  {services.map(svc => {
                    ...
                    const avg = svcReviews.length ? (svcReviews.reduce((s, r) => s + r.rating, 0) / svcReviews.length).toFixed(1) : null;
                    return (
                      <tr key={svc.id} className="group hover:bg-gray-50/60 dark:hover:bg-white/[0.02] transition-colors">
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-lg bg-[#EEF2FF] dark:bg-[#312E81]/30 flex items-center justify-center"><Scissors size={14} className="text-[#6366F1]" /></div>
                            <span className="font-medium text-gray-900 dark:text-white">{svc.name}</span>
                          </div>
                        </td>
                        <td className="px-5 py-4"><span className={cn('rounded-full px-2.5 py-1 text-xs font-medium', CATEGORY_COLORS[svc.category])}>{CATEGORY_LABELS[svc.category]}</span></td>
                        <td className="px-5 py-4"><div className="flex items-center gap-1.5 text-gray-500 dark:text-gray-400"><Clock size={13} />{svc.durationMinutes} dk</div></td>
                        ...
                        <td className="px-5 py-4">
                          {avg ? (
                            <button onClick={() => setReviewSvc(svc)} className="flex items-center gap-1 text-sm font-semibold text-amber-500 hover:underline">
                              <Star size={13} className="fill-amber-400 text-amber-400" />{avg}<span className="text-xs text-gray-400 font-normal">({svcReviews.length})</span>
                            </button>
                          ) : <span className="text-xs text-gray-300 dark:text-gray-600">—</span>}
                        </td>
                        <td className="px-5 py-4">
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            {svcReviews.length > 0 && <button onClick={() => setReviewSvc(svc)} className="flex h-7 w-7 items-center justify-center rounded-lg text-gray-400 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-500 transition-colors"><MessageSquare size={13} /></button>}
                            <button onClick={() => setEditSvc(svc)} className="flex h-7 w-7 items-center justify-center rounded-lg text-gray-400 hover:bg-[#EEF2FF] dark:hover:bg-[#312E81]/30 hover:text-[#6366F1] transition-colors"><Pencil size={13} /></button>
                            <button onClick={() => handleDelete(svc.id)} className="flex h-7 w-7 items-center justify-center rounded-lg text-gray-400 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-500 transition-colors"><Trash2 size={13} /></button>
                          </div>
                        </td>
                      </tr>
                    );
            ...
            <div className="md:hidden divide-y divide-gray-50 dark:divide-gray-700/40">
              {services.map(svc => (
                <div key={svc.id} className="flex items-center gap-3 p-4">
                  <div className="w-10 h-10 rounded-xl bg-[#EEF2FF] dark:bg-[#312E81]/30 flex items-center justify-center shrink-0"><Scissors size={16} className="text-[#6366F1]" /></div>
      ...
      <Modal isOpen={showNew} onClose={() => setShowNew(false)} title="Hizmet Ekle">
        <ServiceForm onSave={handleSaveNew} onCancel={() => setShowNew(false)} />
      </Modal>
      <Modal isOpen={!!editSvc} onClose={() => setEditSvc(null)} title="Hizmet Düzenle">
        {editSvc && <ServiceForm initial={editSvc} onSave={handleSaveEdit} onCancel={() => setEditSvc(null)} />}
      </Modal>
      <Modal isOpen={!!reviewSvc} onClose={() => setReviewSvc(null)} title={reviewSvc ? `${reviewSvc.name} — Değerlendirmeler` : ''} size="md">
        {reviewSvc && <ReviewsModal reviews={reviews.filter(r => r.serviceId === reviewSvc.id)} />}
      </Modal>
    </div>
  );
}
