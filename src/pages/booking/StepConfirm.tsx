...
function SummaryRow({ icon, label, sub, accent }: {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-9 h-9 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center text-indigo-500 shrink-0">
        {icon}
      </div>
      <span className="text-sm text-gray-700 dark:text-gray-300 flex-1 leading-snug">{label}</span>
      {sub && (
        <span className={`text-sm font-bold shrink-0 ${accent ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'}`}>
          {sub}
        </span>
      )}
    </div>
...
export default function StepConfirm() {
  const { services, employees, booking, addAppointment, resetBooking, addNotification } = useStore();
  ...
  // Guard'lar: success modunda booking zaten sıfırlandı, geçme
  if (!success) {
    if (!service || !employee || !booking.date || !booking.startTime) {
      navigate('/book');
      return null;
    }
    if (!currentUser) {
      navigate('/login?next=/book/confirm');
      return null;
    }
  }

  // success=false iken garantili dolu, success=true iken kullanılmıyor
  const endTime = service && booking.startTime
    ? calcEndTime(booking.startTime, service.durationMinutes)
    : '';
  ...
  async function handleConfirm() {
    ...
    if (prefs?.bookingConfirmed !== false) {
      addNotification({
        ...
        body:  `${service.name} · ${dateStr} ${booking.startTime}`,
      });
    }

    // Yarın ya da bugün ise anında hatırlatma gönder
    const daysUntil = Math.ceil(
      (new Date(booking.date).getTime() - new Date().setHours(0,0,0,0)) / 86400000
    );
    if (prefs?.bookingReminder !== false && daysUntil <= 1 && daysUntil >= 0) {
      addNotification({
        type:  'booking_reminder',
        title: daysUntil === 0 ? 'Bugün randevunuz var!' : 'Yarın randevunuz var!',
        body:  `${service.name} · ${employee.name} · ${booking.startTime}`,
      });
    }

    // Önce veriyi kaydet, sonra booking'i sıfırla
    setSuccessData({
    ...
    setLoading(false);
    setSuccess(true);
    // Animasyon başladıktan sonra booking'i sıfırla
    setTimeout(() => resetBooking(), 100);
  }

  /* ── Başarı ekranı — body portal'ı ile her stacking context'in dışında ── */
  if (success) {
    const dateStr = new Date(successData.date).toLocaleDateString('tr-TR', {
      weekday: 'long', day: 'numeric', month: 'long',
    });

    return createPortal(
      <div
        className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-white dark:bg-[#0f1117] px-6"
        style={{ animation: 'sc-bg 0.35s ease both' }}
      >
        <style>{`
        ...
        <div className="relative flex items-center justify-center mb-12">
          <span className="absolute w-36 h-36 rounded-full bg-emerald-400/25 pointer-events-none"
            style={{ animation: 'sc-ring 0.9s cubic-bezier(0.2,0.8,0.4,1) 0.25s both' }} />
          <span className="absolute w-36 h-36 rounded-full bg-emerald-400/15 pointer-events-none"
            style={{ animation: 'sc-ring2 1.1s cubic-bezier(0.2,0.8,0.4,1) 0.15s both' }} />

          {/* İkon dairesi */}
          <div
            className="w-32 h-32 rounded-full bg-emerald-500 flex items-center justify-center shadow-2xl shadow-emerald-500/40"
            style={{ animation: 'sc-icon 0.5s cubic-bezier(0.34,1.56,0.64,1) 0.05s both' }}
          >
            <svg viewBox="0 0 64 64" className="w-16 h-16" fill="none">
              <polyline
                ...
                strokeDasharray="60"
                strokeDashoffset="60"
                style={{ animation: 'sc-check 0.38s ease 0.48s both' }}
              />
            </svg>
          </div>
        </div>
        ...
        <div className="text-center space-y-2" style={{ animation: 'sc-up 0.4s ease 0.72s both', opacity: 0 }}>
          <h2 className="text-[2rem] font-black text-gray-900 dark:text-white tracking-tight leading-none">
            Randevunuz Alındı!
          </h2>
        ...
        <div className="flex flex-col gap-3 w-full max-w-sm mt-10"
          style={{ animation: 'sc-up 0.4s ease 0.92s both', opacity: 0 }}>
          <button
            onClick={() => navigate('/my-appointments')}
            className="w-full py-4 rounded-2xl bg-emerald-500 hover:bg-emerald-600 active:scale-[.98] text-white text-base font-bold shadow-lg shadow-emerald-500/30 transition-all"
          >
            Randevularımı Gör
          </button>
          <button
            onClick={() => navigate('/book')}
            className="w-full py-3 rounded-2xl border border-gray-200 dark:border-gray-700 text-sm font-medium text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
          >
            Yeni Randevu Al
          </button>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        <button
          onClick={() => navigate('/book/datetime')}
          className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors mb-4"
        >
          <ArrowLeft size={15} />
          Geri
        </button>
      ...
      {/* Müşteri bilgileri — sadece göster, form yok */}
      <div className="bg-white dark:bg-[#1a1d27] rounded-2xl border border-gray-100 dark:border-gray-800/60 p-5 space-y-3.5">
      ...
      <button
        ...
        disabled={loading}
        className="w-full py-4 rounded-2xl bg-[#6366F1] hover:bg-[#4F46E5] active:scale-[.98] text-white text-base font-bold shadow-lg shadow-[#6366F1]/30 transition-all disabled:opacity-60 disabled:pointer-events-none flex items-center justify-center gap-2"
      >
        {loading ? (
          <>
      ...
      <p className="text-center text-xs text-gray-400">
        Onayladıktan sonra randevunuz hemen oluşturulur.
      </p>
    </div>
  );
}
