...
export default function StepDateTime() {
  const { services, employees, appointments, booking, setBooking } = useStore();
  ...
  function pickDate(d: string) {
    setSelectedDate(d);
    setBooking({ date: d, startTime: null });
  }

  function selectSlot(time: string) {
    setBooking({ date: selectedDate, startTime: time });
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {list.map(({ time, available }) => (
            <button
              key={time}
              onClick={() => available && selectSlot(time)}
              disabled={!available}
              className={cn(
                ...
                isActive(time)
                  ...
                  : available
                  ? 'bg-white dark:bg-[#1a1d27] border border-gray-100 dark:border-gray-800 text-gray-700 dark:text-gray-300 hover:border-[#6366F1]/40 hover:text-[#6366F1] dark:hover:text-indigo-400 active:scale-95'
  ...
  return (
    <div className="flex flex-col gap-5 pb-4">
      ...
      <div>
        <button
          onClick={() => navigate('/book/employee')}
          className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors mb-3 -ml-0.5"
        >
        ...
        <div className="flex items-end justify-between">
          ...
          {/* İleri ok — bir sonraki güne atla */}
          <div className="flex gap-1">
            <button
              onClick={() => {
                const prev = days[Math.max(0, days.indexOf(selectedDate) - 1)];
                if (prev) pickDate(prev);
              }}
            ...
            <button
              onClick={() => {
                const next = days[Math.min(days.length - 1, days.indexOf(selectedDate) + 1)];
            ...
            <button
              ...
              className={cn(
                ...
                sel
                  ...
                  : 'bg-white dark:bg-[#1a1d27] text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'
              )}
            >
